<?php
/**
	* Plugin Name: Agenvix Core
	* Description: Agenvix core plugin.
	* Plugin URI:  https://themeforest.net/item/agenvix-wallpapers-and-painting-services-wordpress-theme/54844757
	* Version:     1.0.0
	* Author:      bug-finder
	* Author URI:  https://themeforest.net/user/bug-finder/portfolio
	* Text Domain: agenvix-core
...
register_activation_hook(__FILE__, 'agenvix_activate');
...
function agenvix_deactivate() {

}
register_deactivation_hook(__FILE__, 'agenvix_deactivate');
...
/**
 * Agenvix Custom Widget
*/

include_once( PROTINE_WIDGET_PATH . '/agenvix-footer-info.php' );
include_once( PROTINE_WIDGET_PATH . '/agenvix-footer-social.php' );
include_once( PROTINE_WIDGET_PATH . '/agenvix-footer-phone.php' );
include_once( PROTINE_WIDGET_PATH . '/agenvix-footer-email.php' );
include_once( PROTINE_WIDGET_PATH . '/agenvix-footer-lets-talk.php' );
include_once( PROTINE_WIDGET_PATH . '/agenvix-footer-subscriber.php' );
include_once( PROTINE_WIDGET_PATH . '/agenvix-footer-subscriber-2.php' );
include_once( PROTINE_WIDGET_PATH . '/agenvix-footer-address.php' );
include_once( PROTINE_WIDGET_PATH . '/agenvix-menu-list.php' );
include_once( PROTINE_WIDGET_PATH . '/agenvix-footer-logo-info.php' );
include_once( PROTINE_WIDGET_PATH . '/agenvix-footer-logo-info-2.php' );
include_once( PROTINE_WIDGET_PATH . '/agenvix-latest-posts-footer.php' );
include_once( PROTINE_WIDGET_PATH . '/agenvix-blog-post-sidebar.php' );
include_once( PROTINE_WIDGET_PATH . '/agenvix-sidebar-cat-list.php' );
include_once( PROTINE_WIDGET_PATH . '/agenvix-sidebar-tag-list.php' );
include_once( PROTINE_WIDGET_PATH . '/agenvix-nav-menu.php' );
...
function agenvix_enqueue_scripts() {
	...
	wp_enqueue_style( 'agenvix-icon', plugin_dir_url( __FILE__ ) . 'assets/css/agenvix-icon.css', array(), '1.0.0' );
	...
	wp_enqueue_script( 'scroll-to-plugin', plugin_dir_url( __FILE__ ) . 'assets/js/ScrollToPlugin.min.js', array('gsap-js'), '3.14.1', true );
	...
	wp_enqueue_script( 'widget-script', plugin_dir_url( __FILE__ ) . 'assets/js/widget-script.js', [], '1.0', true);
    ...
    wp_localize_script('ajax-filter', 'woocommerce_params', [
        ...
        'nonce'    => wp_create_nonce('provix_filter_products_nonce'),
    ]);
...
add_action( 'wp_enqueue_scripts', 'agenvix_enqueue_scripts', 20 );

/**
 * Admin Scripts
*/

function agenvixcore_admin_scripts() {
	wp_enqueue_style( 'admin-script', plugin_dir_url( __FILE__ ) . 'assets/css/agenvix-admin.css', array(), '1.0.0' );

	wp_enqueue_script('metabox', plugin_dir_url( __FILE__ ) . 'assets/js/metabox.js', ['jquery'], null, true);
}
add_action( 'admin_enqueue_scripts', 'agenvixcore_admin_scripts' );
...
add_filter( 'elementor/fonts/additional_fonts', function( $additional_fonts ) {
    $additional_fonts['Clash Display'] = 'custom_fonts';
    return $additional_fonts;
} );

/**
 * Main Agenvix Core Class
 *
...
final class Agenvix_Core {
	...
	public function admin_notice_missing_main_plugin() {
		...
		$message = sprintf(
			...
			esc_html__( '"%1$s" requires "%2$s" to be installed and activated.', 'agenvix-core' ),
			'<strong>' . esc_html__( 'Agenvix Core', 'agenvix-core' ) . '</strong>',
			'<strong>' . esc_html__( 'Elementor', 'agenvix-core' ) . '</strong>'
		);
	...
	public function admin_notice_minimum_elementor_version() {
		...
		$message = sprintf(
			...
			esc_html__( '"%1$s" requires "%2$s" version %3$s or greater.', 'agenvix-core' ),
			'<strong>' . esc_html__( 'Agenvix Core', 'agenvix-core' ) . '</strong>',
			'<strong>' . esc_html__( 'Elementor', 'agenvix-core' ) . '</strong>',
	...
	public function admin_notice_minimum_php_version() {
		...
		$message = sprintf(
			...
			esc_html__( '"%1$s" requires "%2$s" version %3$s or greater.', 'agenvix-core' ),
			'<strong>' . esc_html__( 'Agenvix Core', 'agenvix-core' ) . '</strong>',
			'<strong>' . esc_html__( 'PHP', 'agenvix-core' ) . '</strong>',
