! function ($) {
    ...
    let e = {
        roundness: "sharp",
        scrollGravity: !1,
        disableOnMobile: true, // New option to disable on mobile
    ...
    class i {
        constructor(i, s) {
            this._defaults = e, this._name = t, this.options = {
                ...e,
                ...s
            }, this.DOM = {}, this.DOM.element = i, this.DOM.$element = $(i), this.DOM.throwables = this.DOM.element.querySelectorAll("[data-t-throwable-el]"), this.onWindowResize = tpDebounce(this.onWindowResize.bind(this), 250), this.bodies = [], this.isMobile = window.innerWidth < this.options.mobileBreakpoint;
        ...
        initStaticLayout() {
            // Show all items in a static grid layout for mobile
            ...
            // Add resize listener to re-initialize if screen size changes
        ...
        createWorld() {
            ...
            // Create engine with better stability settings
            ...
            if (this.mouse.element) {
                this.mouse.element.removeEventListener("wheel", this.mouse.mousewheel);
        ...
        createBodies() {
            this.DOM.throwables.forEach(((t, e) => {
                ...
                Matter.Events.on(this.runner, "tick", (() => {
                    ...
                    // Check if body is sleeping or nearly stopped
                    if (u.isSleeping || (!u.isStatic && Math.abs(u.velocity.x) < 0.1 && Math.abs(u.velocity.y) < 0.1 && Math.abs(u.angularVelocity) < 0.01)) {
                        settledCount++;
                        
                        // After being settled for a while, stop updating
                        if (settledCount > 60 && !isSettled) {
                            isSettled = true;
                            Matter.Body.setVelocity(u, { x: 0, y: 0 });
                            Matter.Body.setAngularVelocity(u, 0);
                            Matter.Sleeping.set(u, true);
                        }
                        
                        // If settled, don't update DOM
                        if (isSettled) return;
                    } else {
                        settledCount = 0;
                        isSettled = false;
                    ...
                    // Update DOM
                    i.style.transform = "translate(-50%, -50%) rotate(" + u.angle.toFixed(3) + "rad)";
                    o(u.position.y.toFixed(2));
                    n(u.position.x.toFixed(2));
                }));
                
                // Reset settled state when body wakes up
                Matter.Events.on(u, 'sleepStart', () => {
                    isSettled = true;
                });
                
                Matter.Events.on(u, 'sleepEnd', () => {
                    isSettled = false;
                    settledCount = 0;
                });
            }))
        ...
        makeScrollGravity() {
            ...
            Matter.Events.on(this.runner, "tick", (() => {
                const e = document.documentElement.scrollTop - document.documentElement.clientTop,
                    i = e - t;
                this.engine.gravity.y = .7 - gsap.utils.clamp(-2, 4, .1 * i);
                t = e
            }))
        }
        updateBoundries() {
            ...
            this.boundStart && (Matter.Body.setPosition(this.boundStart, {
                x: -250,
                y: this.height / 2
            ...
            this.boundEnd && (Matter.Body.setPosition(this.boundEnd, {
                x: this.width + 250,
                y: this.height / 2
            ...
            this.boundBottom && (Matter.Body.setPosition(this.boundBottom, {
                x: 0,
                y: this.height + 250
        ...
        updateBodies() {
            this.DOM.throwables.forEach(((t, e) => {
                ...
                if (i.position.y > this.height) {
                    Matter.Body.setPosition(i, {
                        y: this.height / 2,
                        x: i.position.x
                    });
                    Matter.Body.setVelocity(i, { x: 0, y: 0 });
                }
        ...
        refresh() {
            ...
            if (this.options.disableOnMobile && wasMobile !== this.isMobile) {
                this.destroy();
                location.reload(); // Reload to reinitialize properly
                return;
            }
            
            // Skip refresh if disabled on mobile
            if (this.options.disableOnMobile && this.isMobile) {
                return false;
            }
