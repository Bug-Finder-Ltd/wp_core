...
function render_wishlist() {
    ...
    ob_start();
    ?>
        <div class="zupet-wishlist-page">
            <h3 class="wishlist-title"><?php esc_html_e( 'My Wishlist', 'zupetcore' ); ?></h3>
                    ...
                    <?php foreach ( $wishlist as $product_id ) :
                        ...
                        <div class="product-item">
                            ...
                            <div class="bottom">
                                ...
                                <div class="buy-btn">
                                    <a href="<?php echo esc_url( get_permalink( $product_id ) ); ?>">
                                        <?php esc_html_e('Buy Now', 'zupetcore'); ?>
                                    </a>
                                </div>
                            </div>
                        </div>
            ...
            <?php else : ?>
                <p><?php esc_html_e( 'Your wishlist is empty.', 'zupetcore' ); ?></p>
            <?php endif; ?>
        </div>
    <?php
    return ob_get_clean();
...
add_shortcode( 'zupet_wishlist', 'render_wishlist');


/**
 * Add "Wishlist" tab in WooCommerce My Account page
 */
function zupet_add_wishlist_tab( $items ) {
    // Add new tab after "Dashboard"
    $new_items = [];
    foreach ( $items as $key => $label ) {
        $new_items[ $key ] = $label;
        if ( 'dashboard' === $key ) {
            $new_items['wishlist'] = __( 'Wishlist', 'zupetcore' );
        }
    }
    return $new_items;
}
add_filter( 'woocommerce_account_menu_items', 'zupet_add_wishlist_tab' );

function zupet_add_wishlist_endpoint() {
    add_rewrite_endpoint( 'wishlist', EP_ROOT | EP_PAGES );
}
add_action( 'init', 'zupet_add_wishlist_endpoint' );

function zupet_wishlist_content() {
