...
function render_wishlist() {
    ...
    ob_start();
    ?>
        <div class="raizen-wishlist-page">
            <h3 class="wishlist-title"><?php esc_html_e( 'My Wishlist', 'raizencore' ); ?></h3>
                    ...
                    <?php foreach ( $wishlist as $product_id ) :
                        ...
                        <div class="product-item">
                            ...
                            <div class="bottom">
                                ...
                                <div class="buy-btn">
                                    <a href="<?php echo esc_url( get_permalink( $product_id ) ); ?>">
                                        <?php esc_html_e('Buy Now', 'raizencore'); ?>
                                    </a>
                                </div>
                            </div>
                        </div>
            ...
            <?php else : ?>
                <p><?php esc_html_e( 'Your wishlist is empty.', 'raizencore' ); ?></p>
            <?php endif; ?>
        </div>
    <?php
    return ob_get_clean();
...
add_shortcode( 'raizen_wishlist', 'render_wishlist');


/**
 * Add "Wishlist" tab in WooCommerce My Account page
 */
function raizen_add_wishlist_tab( $items ) {
    // Add new tab after "Dashboard"
    $new_items = [];
    foreach ( $items as $key => $label ) {
        $new_items[ $key ] = $label;
        if ( 'dashboard' === $key ) {
            $new_items['wishlist'] = __( 'Wishlist', 'raizencore' );
        }
    }
    return $new_items;
}
add_filter( 'woocommerce_account_menu_items', 'raizen_add_wishlist_tab' );

function raizen_add_wishlist_endpoint() {
    add_rewrite_endpoint( 'wishlist', EP_ROOT | EP_PAGES );
}
add_action( 'init', 'raizen_add_wishlist_endpoint' );

function raizen_wishlist_content() {
    echo do_shortcode( '[raizen_wishlist]' );
}
add_action( 'woocommerce_account_wishlist_endpoint', 'raizen_wishlist_content' );