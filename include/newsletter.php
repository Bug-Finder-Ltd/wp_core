...
function nd_render_newsletter_form() {
    ...
    if ( isset( $_GET['newsletter'] ) && $_GET['newsletter'] === 'success' ) {
        echo '<p class="newsletter-success">' . esc_html__( 'Thanks for subscribing!', 'raizencore' ) . '</p>';
    }
    ?>
    <form method="post" class="raizen-newsletter-form">
        <input type="email" name="nd_newsletter_email" id="nd_newsletter_email" required placeholder="<?php esc_attr_e( 'Enter your email', 'raizencore' ); ?>">
...
function nd_newsletter_admin_menu() {
    add_menu_page(
        __( 'Newsletter Subscribers', 'raizencore' ),
        __( 'Newsletter', 'raizencore' ),
        'manage_options',
        'nd-newsletter',
...
function nd_newsletter_admin_page() {
    ...
    if ( isset( $_GET['deleted'] ) && $_GET['deleted'] == 1 ) {
        echo '<div class="notice notice-success is-dismissible"><p>'
             . esc_html__( 'Subscriber deleted successfully.', 'raizen-booking' )
             . '</p></div>';
    }
    ?>
    <div class="wrap">
        <h1><?php esc_html_e( 'Newsletter Subscribers', 'raizencore' ); ?></h1>

        <form method="post">
            <?php submit_button( __( 'Export as CSV', 'raizencore' ), 'secondary', 'nd_export_csv' ); ?>
        </form>

        <table class="widefat striped">
            <thead>
                <tr>
                    <th><?php esc_html_e( 'ID', 'raizencore' ); ?></th>
                    <th><?php esc_html_e( 'Email', 'raizencore' ); ?></th>
                    <th><?php esc_html_e( 'Subscribed At', 'raizencore' ); ?></th>
                    <th><?php esc_html_e( 'Action', 'raizencore' ); ?></th>
                </tr>
            </thead>
            <tbody>
                <?php if ( $subscribers ) : ?>
                        ...
                        <tr>
                            ...
                            <td>
                                <a class="button" href="<?php echo wp_nonce_url( admin_url( 'admin.php?page=nd-newsletter&delete=' . $subscriber->id ), 'nd_delete_subscriber_' . $subscriber->id ); ?>" 
                                   onclick="return confirm('<?php esc_attr_e( 'Are you sure you want to delete this subscriber?', 'raizen-booking' ); ?>')">
                                   <?php esc_html_e( 'Delete', 'raizencore' ); ?>
                                </a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                <?php else : ?>
                    <tr>
                        <td colspan="3"><?php esc_html_e( 'No subscribers found.', 'raizencore' ); ?></td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
...
function nd_newsletter_export_csv() {
    if ( isset( $_POST['nd_export_csv'] ) ) {
        ...
        if ( ! $subscribers ) {
            wp_die( __( 'No subscribers to export.', 'raizencore' ) );
        }
...
function nd_handle_delete_subscriber() {
    if ( isset( $_GET['delete'] ) ) {
        ...
        if ( ! isset( $_GET['_wpnonce'] ) || ! wp_verify_nonce( $_GET['_wpnonce'], 'nd_delete_subscriber_' . $id ) ) {
            wp_die( __( 'Security check failed.', 'raizencore' ) );
        }
