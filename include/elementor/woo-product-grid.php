 ...
 * @since 1.0.0
 */
class Raizen_Product_Grid extends \Elementor\Widget_Base {

	public function get_name() {
		return 'raizen-product-grid';
	}

	public function get_title() {
		return __( 'Product Grid', 'raizencore' );
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			'products_per_page',
			[
				'label'   => __( 'Number of Products', 'raizencore' ),
	...
	protected function render() {
		...
		<?php if ( $settings['raizen_design_style']  == 'layout-1' ): ?>
	        ...
	        if ( $loop->have_posts() ) {
	        	?>
				<div class="raizen-product-grid style-one">
					<?php
					$i = 0;
		            while ( $loop->have_posts() ) : $loop->the_post();
						...
						<div class="product-item wow fadeInUp" data-wow-delay="<?php echo esc_attr($delay); ?>ms" data-wow-duration="2000ms">
							...
							<div class="bottom">
								...
								<div class="buy-btn">
									<a href="<?php the_permalink(); ?>"><?php esc_html_e('Buy Now', 'raizencore'); ?></a>
								</div>
							</div>
						</div>
		            <?php $i++; endwhile; ?>
	            ...
	            echo __( 'No products found', 'raizencore' );
	        }
		...
		<?php elseif( $settings['raizen_design_style'] == 'layout-2' ) : ?>
	        ...
	        if ( $loop->have_posts() ) {
	        	?>
				<div class="raizen-product-grid style-two">
					<?php
		            while ( $loop->have_posts() ) : $loop->the_post();
		                global $product;
		                ?>
...
$widgets_manager->register( new Raizen_Product_Grid() );