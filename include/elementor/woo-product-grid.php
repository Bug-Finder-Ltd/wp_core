 ...
 * @since 1.0.0
 */
class Provix_Product_Grid extends \Elementor\Widget_Base {

	public function get_name() {
		return 'provix-product-grid';
	}

	public function get_title() {
		return __( 'Product Grid', 'agenvix-core' );
	}
	...
	protected function register_controls() {
		...
		$this->start_controls_section(
			'products_section',
			[
				'label' => __( 'Products', 'agenvix-core' ),
		...
		$this->add_control(
			'products_per_page',
			[
				'label'   => __( 'Number of Products', 'agenvix-core' ),
	...
	protected function render() {
		...
		<?php if ( $settings['provix_design_style']  == 'layout-1' ): ?>
	        ...
	        if ( $loop->have_posts() ) {
	        	?>
				<div class="provix-product-grid style-one">
					<?php
					$i = 0;
		            while ( $loop->have_posts() ) : $loop->the_post();
						...
						<div class="product-item wow fadeInUp" data-wow-delay="<?php echo esc_attr($delay); ?>ms" data-wow-duration="2000ms">
							...
							<div class="bottom">
								...
								<div class="buy-btn">
									<a href="<?php the_permalink(); ?>"><?php esc_html_e('Buy Now', 'agenvix-core'); ?></a>
								</div>
							</div>
						</div>
		            <?php $i++; endwhile; ?>
	            ...
	            echo __( 'No products found', 'agenvix-core' );
	        }
		...
		<?php elseif( $settings['provix_design_style'] == 'layout-2' ) : ?>
	        ...
	        if ( $loop->have_posts() ) {
	        	?>
				<div class="provix-product-grid style-two">
					<?php
		            while ( $loop->have_posts() ) : $loop->the_post();
		                global $product;
		                ?>
...
$widgets_manager->register( new Provix_Product_Grid() );